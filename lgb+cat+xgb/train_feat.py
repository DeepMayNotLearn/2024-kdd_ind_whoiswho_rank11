...
random.seed(seed)
TARGET_NAME = 'label'
def extract_train_feat(train_author,pid_to_info,venue_all,year_all,authour_org_all):
    # 训练集特征
    train_feats = []
    labels = []
    train_authors = []
    ...
    for id, person_info in tqdm(train_author.items()):  # id:key,person_info:value(name,id)
        ...
        for text_id in person_info['normal_data']:  # 正样本
            ...
            country_name = ['China', 'Japan', 'Canada', 'Germany', 'France', 'Italy', 'Sweden', 'Australia',
                            'Singapore',
                            'Netherlands',
        ...
        for text_id in person_info['outliers']:  # 负样本
            ...
            labels.append(0)
        train_authors_len.append(cnt)
    train_feats = np.array(train_feats)
    labels = np.array(labels)
    print(f"train_feats.shape:{train_feats.shape}")
    train_feats = pd.DataFrame(train_feats)
    train_feats['label'] = labels
    train_feats.columns = ['len_title', 'title_cnt', 'len_abstract', 'abstract_cnt', 'len_keywords', 'keywords_cnt',
                           'len_venue', 'venue_cnt',
                           ...
                           'org_is_null', 'co_org_ratio', 'co_org_cnt', 'not_co_org_ratio', 'not_co_org_cnt',
                           'org_dif_cnt',
                           'co_author_cnt', 'author_org_len', 'author_org_cnt', 'all_org_len', 'all_name_len',
                           'all_org_cnt', 'all_name_cnt',
                           ...
                           ] + [f'country_{i}' for i in range(len(country_cnt))] + [TARGET_NAME]
    train_feats['author_name'] = train_authors
    for col in [f'country_{i}' for i in range(len(country_cnt))]:
        train_feats[f'{col}_ratio'] = train_feats[col] /train_feats['sum_country_cnt']
    return train_feats, train_authors_len