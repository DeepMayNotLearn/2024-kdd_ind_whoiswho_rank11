...
import os
# 从本地文件加载图数据
save_path = 'sci_graph/train_graph_list'
...
for id,person_info in tqdm(train_author.items()):
    ...
    normal_paper=person_info['normal_data']
    outliers_paper=person_info['outliers']
    if (len(normal_paper)+len(outliers_paper))>1600:
        continue
    i+=1
    if i<start_idx:
        continue
    ...
    for index1,row1 in paper_df.iterrows():
        ...
        for index2,row2 in paper_df.iterrows():
            ...
            paper2=row2['id']
            if (len(normal_paper)+len(outliers_paper))>800:
    ...
    if len(G.edges()) == 0:
       continue
    device='cpu'
    G_data=dgl.from_networkx(G,node_attrs=['x','y'],edge_attrs=['edge_attr']).to(device)
